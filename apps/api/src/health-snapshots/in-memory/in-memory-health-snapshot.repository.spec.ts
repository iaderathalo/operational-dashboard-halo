...
describe('InMemoryHealthSnapshotRepository', () => {
    it('returns the most recent snapshots first, windowed by limit, scoped to the app', async () => {
        ...
        await repo.insertSnapshot(snap({ recordedAt: '2026-06-14T00:00:00.000Z', status: 'GREEN' }));
        await repo.insertSnapshot(snap({ recordedAt: '2026-06-14T00:05:00.000Z', status: 'AMBER' }));
        await repo.insertSnapshot(snap({ recordedAt: '2026-06-14T00:10:00.000Z', status: 'RED' }));
        await repo.insertSnapshot(snap({ applicationId: 'other-app', recordedAt: '2026-06-14T09:00:00.000Z' }));
