...
> frontmatter are for traceability only — consult them only if you need narrative rationale or prose
> color this contract intentionally omits.
...
Name which (or which combination) applies, who is affected, and the backdrop that makes it matter
...
- **CAP-1**
  ...
  - **success:** {Testable or demonstrable criterion. Something a test or a real demonstration can
    decide.}
...
- {One or two sentences. World-change moment, not dashboard. Concrete enough to write a test or run
...
- {Statement of fact the Spec proceeded under, e.g. "Assumed mobile-first since input mentioned GPS
  but no platform."}
