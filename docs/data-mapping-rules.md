...
| Top risks — apps to review                   | The 5 riskiest monitored apps right now                                        | ✅ Real            | Each sync | Monitoring (Datadog)       | Health severity ×2 + burn-rate severity, top 5; excludes apps with no Datadog data                                               | derived from `health` / `burnRate`             |
...
| **Burn Rate** (column)                       | How fast it's using its allowed downtime budget                                | ✅ Real            | Each sync | Datadog SLO                | Budget-consumption rate over the 30-day window. Below 1.0x = burning within budget; 1.0x or higher = burning faster than allowed | `app.burnRate`                                 |
| **Perception** (column)                      | (User-experience score)                                                        | ⚪ Sample (hidden) | —         | —                          | Hidden until defined                                                                                                             | `placeholderColumns.perception` (planned)      |
| **Total External Users**                     | External user count                                                            | ✅ Real            | Each sync | App catalog (PlanView)     | Direct value; ~40% of PlanView records carry 0 for both counts; 0 is the source value, not a pipeline gap.                       | `app.totalExternalUsers`                       |
| **Total Internal Users**                     | Internal user count                                                            | ✅ Real            | Each sync | App catalog (PlanView)     | Direct value; ~40% of PlanView records carry 0 for both counts; 0 is the source value, not a pipeline gap.                       | `app.totalInternalUsers`                       |
...
| Error Budget             | How much downtime it can still afford this month | ✅ Real                    | Each sync | Datadog SLO                 | % of the allowed downtime budget still unused; 100% = none spent, 0% = SLA breached. Formula: ((uptime30d − target) / (100 − target)) × 100 | `.../detail` → `errorBudget`          |
...
| Uptime & Error Budget       | 30-day uptime + budget                               | ✅ Real                 | Each sync | Datadog SLO                     | 30-day uptime % + % of error budget remaining                                                 | `.../detail`                                    |
...
update on Datadog sync (today a manual `POST /api/v1/internal/sync/datadog`, with
`.../sync/planview` for the catalog; a scheduled cadence is planned), freshness shown via
`lastSyncAt`. **Endpoints:** `GET /api/v1/dashboard/portfolio`, `.../portfolio/apps/{id}`,
`.../portfolio/apps/{id}/detail`, `.../portfolio/apps/{id}/health-history`.
